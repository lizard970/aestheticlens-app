              ...
              {!result ? <div className="flex min-h-[520px] flex-col items-center justify-center text-center"><div className="mb-4 flex size-12 items-center justify-center rounded-2xl border border-cyan-300/15 bg-cyan-300/6 text-cyan-200"><MessageSquareText className="size-6" /></div><h2 className="text-lg font-medium">分析结果将在这里展开</h2><p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">上传图片后，系统会依次展示任务进度、五维判断、证据和反馈入口。</p></div> : <>
                ...
                <Tabs defaultValue={result.dimensions[0].code}>
                  ...
                  {result.dimensions.map((dimension) => <TabsContent key={dimension.code} value={dimension.code} className="pt-4"><div className="space-y-4">
                    <div className="rounded-xl bg-white/3 p-4"><p className="text-xs font-medium text-cyan-200">观察</p><p className="mt-2 leading-6">{dimension.observation}</p></div>
                    <div className="rounded-xl border border-amber-300/12 bg-amber-300/5 p-4"><div className="flex items-center justify-between"><p className="text-xs font-medium text-amber-200">解释</p><span className="font-mono text-xs text-muted-foreground">证据支持 {(dimension.confidence * 100).toFixed(0)}%</span></div><p className="mt-2 leading-6">{dimension.interpretation}</p></div>
                    <div><p className="mb-2 text-xs font-medium text-muted-foreground">证据引用</p><div className="flex flex-wrap gap-2">{dimension.evidence.map((item) => <Badge key={item} variant="outline" className="border-white/10">{item}</Badge>)}</div></div>
                  </div></TabsContent>)}
                </Tabs>
