...
function feature(
  ...
  status: FeatureResult['status'] = 'succeeded',
): FeatureResult {
  return {
    extractor_code: code,
    ...
    values,
    artifacts: [],
    provenance: { profile_source: 'assumed sRGB', reference_white: 'D65' },
    warnings: [],
    error_detail: status === 'failed' ? 'fixture failure' : null,
  };
}
...
const apiResponse = {
  ...
  features: [
    ...
    feature('global_tonal_contrast', {
      lstar_p95_p05_span: 81.25,
      lstar_iqr: 42.5,
      lstar_standard_deviation: 24.75,
    }),
    feature('multiscale_local_contrast', {
      scales: [{ scale_fraction: 0.03125, energy_rms: 0.1875 }],
    }),
    feature('tonal_occupancy', {
      configuration_version: '1.0.0',
      shadow_lstar_max: 20,
      highlight_lstar_min: 80,
      shadow_share: 0.45,
      midtone_share: 0.4,
      highlight_share: 0.15,
    }),
  ],
};
...
describe('ComputationalFeatures', () => {
  it('renders concrete values from a non-empty API features response', () => {
    render(
      <ComputationalFeatures
        ...
        completionStatus={apiResponse.completion_status}
        resultWarnings={apiResponse.warnings}
      />,
    );
    expect(
      screen.getByRole('region', { name: '真实计算' }),
    ).toBeInTheDocument();
    expect(screen.getByText('平均感知明度 L*')).toBeInTheDocument();
    expect(screen.getByText('53.59')).toBeInTheDocument();
    expect(screen.getByText('全局色调跨度 p95-p05')).toBeInTheDocument();
    expect(screen.getByText('81.25')).toBeInTheDocument();
    expect(screen.getByText('尺度 3.1%')).toBeInTheDocument();
    expect(screen.getByText('RMS 0.19')).toBeInTheDocument();
    expect(
      screen.getByText('Tone distribution · 明暗层级'),
    ).toBeInTheDocument();
  ...
  it('shows successful values and failed extractors for a partial result', () => {
    render(
      <ComputationalFeatures
        features={[
          ...
          feature('fixture_failure', {}, 'failed'),
        ]}
    ...
    expect(screen.getByText('52.13')).toBeInTheDocument();
    expect(
      screen.getByText(/提取失败 · fixture_failure：fixture failure/),
    ).toBeInTheDocument();
  });
