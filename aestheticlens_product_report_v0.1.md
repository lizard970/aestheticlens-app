...
AestheticLens 是一个面向内容创作者、AI 产品经理和美学评测人员的多模态产品。它将视频或图片中的构图、色彩、光影、空间和风格判断，转化为可解释、可检索、可评测、可复用的结构化知识。
...
第一阶段只实现单张图片的端到端闭环，但数据结构、服务边界和 API 从第一天支持未来的视频、知识库、评测和视觉配方模块。
...
| Infrastructure | 数据库、对象存储、模型、视频工具适配 | 向上泄漏供应商结构 |
...
图片素材在领域层可直接作为分析目标，不必伪造为视频帧；统一目标由 `AnalysisTarget` 表达。
...
- `id`；
- `target_type`：`asset | frame | shot | video`；
- `target_id`；
- `analysis_profile_id`；
- `requested_by`；
...
```json
{
  "capabilities": [
    ...
    {"code": "video_shot_detection", "status": "not_implemented", "version": null}
  ]
}
```
...
| `POST` | `/projects/{project_id}/assets` | 上传素材并创建记录 |
...
```json
{
  ...
  "asset_type": "image",
  "status": "ready",
  "media": {"width": 1920, "height": 1080, "duration_ms": null},
...
| 模块 | 路径 |
|---|---|
| 视频取帧 | `/assets/{asset_id}/frame-extraction-jobs` |
| 镜头检测 | `/assets/{asset_id}/shot-detection-jobs` |
...
后续依次进入知识库、评测、视频、视觉配方和 Skill，不并行堆砌半成品模块。
...
7. 如何从单图架构平滑扩展到视频，而不重写核心系统。

建议简历表达：

> 设计并实现多模态视频美学分析与视觉知识检索平台，将构图、色彩、光影等主观判断转化为结构化标签、计算特征和可追溯证据；搭建模型直出、计算增强、RAG 增强三组评测实验，通过人工盲评、规则一致性和幻觉检测验证方案效果，并将确认案例沉淀为可复用视觉配方。
...
| 媒体处理 | OpenCV，视频阶段加入 FFmpeg | 阶段 4 前 |
...
阶段 1A 不引入真实模型密钥，也不引入视频处理，以便先验证产品交互、数据契约和状态设计。
